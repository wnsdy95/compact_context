...
Usage:
    python -m ailang_ir compress [-s SPEAKER] [FILE]   Compress text to semantic codes
    python -m ailang_ir ingest [FILE]                   Ingest LLM-produced codes
    python -m ailang_ir export [-n N] FILE              Export stored memories as LLM format
    python -m ailang_ir spec                            Print LLM format spec
    python -m ailang_ir interactive                     Interactive compression REPL
...
def cmd_compress(args: argparse.Namespace) -> None:
    ...
    if args.format in ("llm", "both"):
        from ailang_ir.llm.codec import LLMCodec
        codec = LLMCodec()
        compressed = "\n".join(codec.encode(r.frame) for r in results)
    else:
...
def cmd_export(args: argparse.Namespace) -> None:
    ...
    output = pipe.export_context(args.n)
    print(output)
...
def cmd_interactive(args: argparse.Namespace) -> None:
    ...
    print("Commands: /speaker <u|s|a> | /export [n] | /stats | /quit", file=sys.stderr)
